...
object Philosopher {
  ...
  object Bomb extends PhilosopherProtocol
}
...
class Philosopher(name: String, left: ActorRef, right: ActorRef) extends Actor {
  ...
  def thinking: Receive = handleBomb orElse {
    case Eat =>
      become(hungry)
  ...
  def hungry: Receive = handleBomb orElse {
    case Taken(`left`) =>
  ...
  def waiting_for(chopstickToWaitFor: ActorRef, otherChopstick: ActorRef): Receive = handleBomb orElse {
  ...
  def denied_a_chopstick: Receive = handleBomb orElse {
  ...
  def eating: Receive = handleBomb orElse {
    case Think =>
